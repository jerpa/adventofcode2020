...
func main() {
	...
	img := []string{}
	for y := minY; y <= maxY; y++ {
		for l := 1; l < len(m[0].data)-1; l++ {
			row := ""
			for x := minX; x <= maxX; x++ {
				d := res[coordID(x, y)].data
	...
	monsterData := []c.Point{}
	for y := range monster {
		for x := 0; x < len(monster[y]); x++ {
			if monster[y][x] == '#' {
				monsterData = append(monsterData, c.Point{X: x, Y: y})
			}
		}
	}
	...
	for f := 0; f < 4; f++ {
		t.flipX()
		for i := 0; i < 4; i++ {
			for y := 0; y < len(t.data)-len(monster); y++ {
				for x := 0; x < len(t.data[0])-len(monster[0]); x++ {
					match := true
					for _, r := range monsterData {
						if t.data[y+r.Y][x+r.X] != '#' {
							match = false
							break
						}
					}
					if match {
						for _, r := range monsterData {
							ch := []rune(t.data[y+r.Y])
							ch[x+r.X] = 'O'
							t.data[y+r.Y] = string(ch)
						}
					}
				}
			}
